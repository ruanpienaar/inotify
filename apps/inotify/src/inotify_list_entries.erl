...
-define(STATE,inotify_list_entries_state).
-record(?STATE,{}).
...
handle_html(Req, State) ->
    ...
    {atomic,RECS}
     = mnesia:transaction(fun() -> 
        [ begin 
            [Rec] = mnesia:read(inotify_entry, Key),

            N = io_lib:format("~p", [calendar:now_to_datetime(Rec#inotify_entry.now)]),
            "<tr><td>"++N++"</td><td>"++Rec#inotify_entry.dir++
            "</td><td>"++Rec#inotify_entry.actions++"</td><td>"++
            Rec#inotify_entry.file++"</td></tr>"
          end || Key <- mnesia:all_keys(inotify_entry) 
        ]
    end),

    HTML=
        "<table>"++
        "<tr><th>datetime</th><th>Directory</th><th>Action</th><th>File</th></tr>"++
        lists:flatten(RECS) ++
        "</table>",
    {list_to_binary(HTML), Req, State}.
